...
// find selected path in $files
$found = null;
foreach ($files as $f) { if (basename($f) === $selected) { $found = $f; break; } }
if ($found) $selectedPath = $found;
if (!$selectedPath || !file_exists($selectedPath)) $selectedPath = reset($files);
...
<head>
  ...
  <style>
    ...
    .sidebar{width:320px;position:fixed;left:0;top:0;bottom:0;background:#111;color:#fff;padding:12px;overflow:auto}
    .content{margin-left:340px;padding:24px}
    a{color:#4ea3ff}
