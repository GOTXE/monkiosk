...
SOURCE_SCRIPT="$REPO_ROOT/kiosks_report/debian/report_status.sh"
OPENRC_TEMPLATE="$SCRIPT_DIR/kioskmonitoring.openrc"
...
require_root
require_cmd install cp rc-update rc-service sed
...
if [ ! -f "$SOURCE_SCRIPT" ]; then
    echo "ERROR: no se encuentra script fuente: $SOURCE_SCRIPT" >&2
    exit 1
fi
if [ ! -f "$OPENRC_TEMPLATE" ]; then
    echo "ERROR: no se encuentra plantilla OpenRC: $OPENRC_TEMPLATE" >&2
    exit 1
fi
